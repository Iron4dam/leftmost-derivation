...
max_steps = 3000    # max steps the agent can execute per episode
max_episodes = 10e4  # max number of episodes
decay_in = 5e4
decay_to = 0.
...
value_one = False
load = True
load_path = '/home/adamxinyuyang/Documents/leftmost-derivation/saved_models/transfer_load_False_3_disks_6_nt_6_pt_-0.001_0.000_-0.001_reward_1000_steps_False_load_0_seed_0.00_decay_lr/100000.pt'
...
class Policy(nn.Module):
    def __init__(self, action_space=args.action_space_dim,
        ...
        # encoders, NNs
        self.state_emb_z = nn.Linear(5*3, self.z_dim)
    ...
    def forward(self, state, use_mean=True):
        ...
        state_emb_full = torch.zeros(1,5*3).to(device)
        state_emb_full[:,:self.state_space] = state_emb

        self.z = F.relu(self.state_emb_z(state_emb_full))
        # self.z = self.state_emb_z(state_emb_full)
        z = self.z
...
def main():
    ...
    model_folder_name = 'transfer_load_%s_%d_disks_%d_nt_%d_pt_%.3f_%.3f_%.3f_reward_%d_steps_%s_load_%d_seed_%.2f_decay/'\
        %(args.load, args.state_space_dim, args.nt_states, args.pt_states, args.rule_reward, args.positive_reward, args.negative_reward, args.max_steps, args.load, args.seed, args.decay_to)
    ...
    for i_episode in range(1, int(args.max_episodes) + 1):  # count(1):
        if args.decay:
            ...
            # lr = linear_anneal(counter=i_episode, start=args.lr, final=1e-4, in_steps=decay_in)
