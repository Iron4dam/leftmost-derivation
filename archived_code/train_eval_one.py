...
max_steps = 2000    # max steps the agent can execute per episode
max_episodes = 40000  # max number of episodes
...
class Policy(nn.Module):
    def __init__(self, action_space=args.action_space_dim,
        ...
        # policy networks
        if value_one:
            self.rule_mlp = nn.Sequential(nn.Linear(symbol_dim + self.z_dim, symbol_dim),
                                        nn.ReLU(),
                                        nn.Linear(symbol_dim, self.root_rule_num+self.nt_nt_rule_num+action_space))
        else:
    ...
    def forward(self, state, use_mean=True):
        ...
        if nt == 0:
            ...
            if self.value_one:
                root_rule_score = self.rule_mlp(root_emb)[:,:self.root_rule_num].squeeze()
            else:
            ...
            if nt < self.nt_states:   # nt is a nt
                ...
                if self.value_one:
                    nt_rule_score = self.rule_mlp(nt_emb)[:,self.root_rule_num:self.root_rule_num+self.nt_nt_rule_num].squeeze()
                else:
                ...
                if self.value_one:
                    pt_rule_score = self.rule_mlp(pt_emb)[:,self.root_rule_num+self.nt_nt_rule_num:self.root_rule_num+self.nt_nt_rule_num+self.action_space].squeeze()
                else:
...
def main():
    ...
    writer = SummaryWriter(log_dir='/home/xy2419/leftmost-derivation/runs/NN_%s_value_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_steps_%s_load/' 
        %(args.value_one, args.entropy_factor, args.nt_states, args.pt_states, args.decay, args.rule_reward, args.positive_reward, args.negative_reward, args.state_space_dim, args.max_steps, args.load))
    ...
    for i_episode in range(1, args.max_episodes + 1):  # count(1):
        if args.decay:
            entropy_factor = linear_anneal(counter=i_episode, start=args.entropy_factor, final=0.01, in_steps=2e4)
        ...
        if i_episode > 4:
            ...
            if t < min_step:
                torch.save(model.state_dict(), '/home/xy2419/leftmost-derivation/saved_models/ac_%s_value_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_steps_%s_load.pt' 
                %(args.value_one, args.entropy_factor, args.nt_states, args.pt_states, args.decay, args.rule_reward, args.positive_reward, args.negative_reward, args.state_space_dim, args.max_steps, args.load))
                min_step = t
                ...
                if i_episode < 20:
                    ...
                    with open('/home/xy2419/leftmost-derivation/plot/NN_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_steps_%s_load.txt' 
                    %(args.entropy_factor, args.nt_states, args.pt_states, args.decay, args.rule_reward, args.positive_reward, args.negative_reward, args.state_space_dim, args.max_steps, args.load), "a") as text_file:
                        text_file.write('graph')
                        text_file.write('\n')
                        text_file.write('{')
                        text_file.write(model.graph)
