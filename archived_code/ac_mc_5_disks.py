...
max_episodes = 50000  # max number of episodes
...
def main():
    ...
    writer = SummaryWriter(log_dir='/home/xy2419/leftmost-derivation/runs/ac_mc_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_steps/' 
    ...
    for i_episode in range(1, args.max_episodes + 1):  # count(1):
        if args.decay:
            entropy_factor = linear_anneal(counter=i_episode, start=args.entropy_factor, final=0.01, in_steps=4e4)
        ...
        if i_episode > 4:
            ...
            if t < min_step:
                torch.save(model.state_dict(), '/home/xy2419/leftmost-derivation/saved_models/ac_mc_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_steps.pt' 
                %(args.entropy_factor, args.nt_states, args.pt_states, args.decay, args.rule_reward, args.positive_reward, args.negative_reward, args.state_space_dim, args.max_steps))
