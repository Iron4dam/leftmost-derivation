...
def main():
    ...
    writer = SummaryWriter(log_dir='/home/xy2419/leftmost-derivation/runs/relu_head_head_%s_value_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_steps/' 
    ...
    for i_episode in range(1, args.max_episodes + 1):  # count(1):
        ...
        if i_episode > 4:
            ...
            if t < min_step:
                torch.save(model.state_dict(), '/home/xy2419/leftmost-derivation/saved_models/relu_head_head_%s_value_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_steps.pt' 
                %(args.value_one, args.entropy_factor, args.nt_states, args.pt_states, args.decay, args.rule_reward, args.positive_reward, args.negative_reward, args.state_space_dim, args.max_steps))
                min_step = t
                print('episode %d, step taken %d, micro length %d, model saved' %(i_episode,t,len(micro_list)))

                if i_episode < 50:
                    with open('/home/xy2419/leftmost-derivation/plot/ac_mc_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_steps.txt' 
                    ...
                    with open('/home/xy2419/leftmost-derivation/plot/relu_head_head_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_steps.txt' 
