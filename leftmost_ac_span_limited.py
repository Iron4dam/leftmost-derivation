    ...
    def forward(self, nt, state, use_mean=True):
        ...
        if nt == 'S':
            ...
            if nt < self.nt_states:   # nt is a nt
                ...
                nt1 = int((rule+1)//(self.pt_states + self.nt_states) + self.action_space - 1)
                nt2 = int((rule+1)%(self.pt_states + self.nt_states) + self.action_space - 1)
...
class Tree:
    def __init__(self, parent_tree, 
        ...
        # just for initialisation, change later
        if self.left_node_type == 'T':
            self.expanded = True
        else:
            self.expanded = False
...
def main():
    ...
    writer = SummaryWriter(log_dir='/home/xy2419/leftmost-derivation/runs/span_new_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d/' 
    ...
    for i_episode in range(1, args.max_episodes+1):  # count(1):
        if decay:
            if i_episode == 200:
                entropy_factor = 9/10 * entropy_factor
            elif i_episode == 400:
                entropy_factor = 9/10  * entropy_factor
            elif i_episode == 600:
                entropy_factor = 9/10 * entropy_factor
        ...
        for t in range(args.max_steps):    # tqdm(range(1, max_steps)):
            ...
            log_prob, rule_prob, left_node, right_node = model(parent_node, state)
            ...
            # child_tree.value = old_value
            # child_tree.action_log_prob = log_prob
            # child_tree.action_dist = rule_prob

            # reward = -0.001   # for NT->NT/PT,NT/PT rules
        ...
        if success_count > 200:
            ...
            torch.save(model.state_dict(), '/home/xy2419/leftmost-derivation/saved_models/span_new_%.3f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d.pt' 
        ...
        writer.add_scalar('1 Episode reward', ep_reward, i_episode)
        writer.add_scalar('2 Steps per episode', t, i_episode)

        writer.add_scalar('3 Actor loss', np.mean(episode_actor_loss), i_episode)
        writer.add_scalar('4 Entropy loss', np.mean(episode_entropy_loss), i_episode)
        writer.add_scalar('5 Critic loss', np.mean(episode_critic_loss), i_episode)

        if done:
            writer.add_scalar('6 Episode success', 1, i_episode)
        else:
            writer.add_scalar('7 Episode success', 0, i_episode)

        writer.add_scalar('8 Number of trees', num_tree, i_episode)
        writer.add_scalar('9 Length of terminal sequence', len(micro_list), i_episode)
        writer.add_scalar('10 Max node count', max_node_count, i_episode)
