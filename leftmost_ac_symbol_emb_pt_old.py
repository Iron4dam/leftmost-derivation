...
def main():
    ...
    writer = SummaryWriter(log_dir='/home/xy2419/leftmost-derivation/runs/old_%.3f_%.1f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_%.3f_scale/' 
    ...
    for i_episode in range(1, args.max_episodes + 1):  # count(1):
        if args.decay:
            if i_episode == 100:
                entropy_factor = entropy_factor * args.decay_scale
            elif i_episode == 200:
                entropy_factor = entropy_factor * args.decay_scale
            elif i_episode == 300:
                entropy_factor = entropy_factor * args.decay_scale
        ...
        if success_count > 200:
            ...
            torch.save(model.state_dict(), '/home/xy2419/leftmost-derivation/saved_models/old_%.3f_%.1f_entropy_%d_nt_%d_pt_%s_decay_%.3f_%.3f_%.3f_reward_%d_disks_%d_%.3f_scale.pt' 
            %(args.entropy_factor, args.decay_scale, args.nt_states, args.pt_states, args.decay, args.rule_reward, args.positive_reward, args.negative_reward, args.state_space_dim, args.max_steps, args.scale))
